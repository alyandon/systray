...
// Keep track of last generated temp file to remove when app quits
static char* global_last_icon;
...
gboolean do_set_icon(gpointer data) {
	...
	// remove icon when changing
	int ret = unlink(global_last_icon);
	if (ret == -1) {
		printf("failed to remove temp icon file %s: %s\n", global_last_icon, strerror(errno));
	}
	global_last_icon = temp_file_name;
	return FALSE;
}
